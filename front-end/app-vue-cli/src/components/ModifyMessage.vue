<template>
    <div id="message-modify" class="mt-5 d-flex justify-content-center align-items-center">
        <div class="container col-10 col-md-8 col-lg-8 col-xl-6">
        <form @submit.prevent="" class="bg-custum p-4 rounded" enctype="multipart/form-data">
            <div>
            <p class="p"><pre>{{preview}}</pre></p>
            <textarea @focus="updateTextareaHeight" @input="updateTextareaHeight" v-model="preview" name="message" ref="textarea" id="textarea" class="rounded p-2" placeholder="Écrivez un commentaire..." autocomplete="off" cols="100">
            </textarea>
...
export default {
    ...
    data() {
        return {
            preview: "",
            file: "",
            messageId: parseInt(sessionStorage.getItem('messageId'))
        }
    },
    methods: {
        focusArea() {
        ...
        sendMessage() {
            ...
            axios.put('http://localhost:5000/message/update', data)
            .then(() => {
    ...
    mounted() {
        ...
        let messageContent = sessionStorage.getItem('messageContent')
        this.preview = messageContent
